...
function parseRuntimeOrigin(value) {
  let parsed;
  try {
...
function normalizedProfile(profile) {
  const parsed = parseRuntimeOrigin(profile?.origin);
  return {
    ...profile,
    origin: parsed.origin,
    runtimeServiceId: parsed.host,
    coreBasePath: normalizePath(profile?.coreBasePath, DEFAULT_CORE_BASE_PATH).replace(/\/$/, ''),
    loginPath: normalizePath(profile?.loginPath, DEFAULT_LOGIN_PATH),
...
async function fetchSameOrigin(state, profileInput, path, options = {}) {
  ...
  const validation = await validateRuntimeOrigin(profile.origin, options);
  ...
  try {
    ...
    response = options.fetchImpl
      ...
      : await pinnedHttpsFetch(url, init, validation);
  } catch (error) {
  ...
  if (!response.ok) throw new RuntimeClientError('RUNTIME_HTTP_ERROR', `Runtime returned HTTP ${response.status}.`, 502);
  for (const value of setCookieValues(response.headers)) await jar.setCookie(value, url.toString(), { ignoreError: true });
...
async function initializeRuntime(state, profile, options = {}) {
  await fetchSameOrigin(state, profile, normalizedProfile(profile).loginPath, {
    ...options,
    method: 'GET',
    refererPath: normalizedProfile(profile).loginPath,
...
async function postCore(state, profileInput, endpoint, payload, options = {}) {
  const profile = normalizedProfile(profileInput);
  const result = await fetchSameOrigin(state, profile, `${profile.coreBasePath}/data-provider/${endpoint}`, {
    ...options,
    method: 'POST',
    payload,
    refererPath: options.login === true ? profile.loginPath : profile.appRefererPath,
    headers: {
      ...(options.prostage ? { prostage: String(options.prostage) } : {}),
      ...(options.headers || {}),
    },
  });
...
async function runtimeWhoAmI(state, profile, options = {}) {
  return postCore(state, profile, 'get-data-source', {
    serviceId: normalizedProfile(profile).runtimeServiceId,
